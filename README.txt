...
We used the 'SMOOTHL1LOSS' from torch.nn in the 'pacmanDQN_Agents.py' file to compute our loss, which creates a criterion that uses a squared term if the absolute element-wise error falls below beta and an L1 term otherwise. 
...
Then, we can take experiences from storage and replay them to the agent so that it can learn from them and take better actions in the future.

As we start from the beggining the graphs of loss/iteration starts to converge after some time and then there's a point where the graphs converge and after a that certain point even if we train the model more and more it doesn't effect the graphs anymore.

I tried the classical DQN architecture uses a single NN to predict directly the value of all possible actions.

DQN.py and pacmanDQN_Agents.py are the two main files that we added which does most of our NN work. 
